...
export const calculateTrade = ({
  ...
  const decimal = amountDecimals + priceDecimals;
  ...
  if (orderType === 'market' && side === 'buy') {
    ...
    subtotal = amount.multipliedBy(price).dp(decimal, BigNumber.ROUND_DOWN);
    totalBaseTokens = amount;
  }

  const estimatedPrice = orderType === 'market' ? price : new BigNumber(0);

  tradeFee = subtotal.multipliedBy(feeRate).dp(decimal, BigNumber.ROUND_DOWN);
