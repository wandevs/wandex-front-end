...
const recoverMarket = (markets, id) => {
  let market, num = markets.length;
  for (let i = 0; i < num; i++) {
    market = markets[i];
    if (market.id === id) {
      return market;
...
export default (state = initialState, action) => {
  switch (action.type) {
    case 'LOAD_MARKETS':
      action.payload.markets= action.payload.markets.reverse();
      state = state.setIn(['markets', 'data'], List(action.payload.markets));
      if (!state.getIn(['markets', 'currentMarket'])) {
        let currentMarket = null;
        let currentMarketId = localStorage.getItem('currentMarketId');
        if (currentMarketId) {
          currentMarket = recoverMarket(action.payload.markets, currentMarketId);
        }
        state = state.setIn(['markets', 'currentMarket'], currentMarket || action.payload.markets[0]);
      }
      return state;
    case 'UPDATE_CURRENT_MARKET': {
      ...
      localStorage.setItem('currentMarketId', currentMarket.id);
    ...
    case 'PRICE_UPDATE':{
        ...
        state = state.setIn(['tokenPrices', 'price'], price);
        return state;
      }
    default:
      return state;
